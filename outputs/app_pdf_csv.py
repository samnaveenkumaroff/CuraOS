...
def main():
    ...
    if uploaded_file:
        ...
        tabs = st.tabs(["📅 Timeline", "📄 Page View", "🔍 Entity Summary", "📊 Analytics", "📥 Export"])
        ...
        with tabs[0]:
            ...
            if timeline_fig:
                ...
                if timeline_data:
                    ...
                    timeline_csv = generate_timeline_csv(entities)
                    st.download_button(
                        label="📊 Export Timeline as CSV",
                        data=timeline_csv,
                        file_name="document_timeline.csv",
                        mime="text/csv"
                    )
            else:
        ...
        with tabs[1]:
            ...
            with col1:
                ...
                # Export single page entity as CSV
                single_page_entity = [entity]
                single_page_csv = entities_to_csv(single_page_entity)
                st.download_button(
                    label="📄 Export Page Data as CSV",
                    data=single_page_csv,
        ...
        with tabs[2]:
            ...
            if search_term:
                ...
                if not filtered_df.empty:
                    # Convert filtered dataframe back to entities format
                    filtered_indices = filtered_df.index.tolist()
                    filtered_entities = [entities[i] for i in filtered_indices]
                    
                    filtered_csv = entities_to_csv(filtered_entities)
                    st.download_button(
                ...
                # Export full entity table
                full_csv = entities_to_csv(entities)
                st.download_button(
                    label="📥 Export Full Entity Table as CSV",
                    data=full_csv,
            ...
            with col3:
                ...
                if lab_tests:
                    ...
                    # Add lab tests CSV export option
                    lab_tests_csv = generate_lab_tests_csv(entities)
                    st.download_button(
                        label="🧪 Export Lab Tests as CSV",
                        data=lab_tests_csv,
                        file_name="lab_tests.csv",
                        mime="text/csv"
                    )
        ...
        with tabs[4]:
            ...
            export_type = st.radio(
                ...
                ["Simple CSV", "Detailed CSV", "Filtered CSV", "Timeline CSV", "Lab Tests CSV"],
                horizontal=True
            )
            
            if export_type == "Simple CSV":
                st.info("Simple CSV contains basic entity information in a single table format.")
                simple_csv = entities_to_csv(entities)
                st.download_button(
                ...
                # Preview
                st.markdown("### Preview")
                preview_df = pd.read_csv(io.StringIO(simple_csv), nrows=5)
                st.dataframe(preview_df, use_container_width=True)
                
            elif export_type == "Detailed CSV":
                st.info("Detailed CSV includes multiple sections with entity summaries and statistics.")
                detailed_csv = generate_detailed_csv(entities)
                st.download_button(
                    label="📥 Download Detailed CSV",
                    data=detailed_csv,
                    file_name="detailed_medical_report.csv",
                    mime="text/csv"
                )
                
                # Preview
                st.markdown("### Preview (First Section)")
                preview_lines = detailed_csv.split("\n\n")[0].split("\n")
                preview_text = "\n".join(preview_lines[:10])
                st.code(preview_text)
                
            elif export_type == "Filtered CSV":
                st.info("Create a customized export by filtering entities.")
                ...
                with col1:
                    filter_provider = st.selectbox(
                        "Filter by Provider",
                        ["All"] + list(set(e["provider"] for e in entities if e["provider"]))
                    )
                
                with col2:
                    filter_facility = st.selectbox(
                        "Filter by Facility",
                        ["All"] + list(set(e["facility"] for e in entities if e["facility"]))
                    )
                
                # Apply filters
                filters = {
                    "provider": filter_provider if filter_provider != "All" else None,
                    "facility": filter_facility if filter_facility != "All" else None
                }
                
                filtered_entities = entities.copy()
                for field, value in filters.items():
                    if value:
                        filtered_entities = [e for e in filtered_entities if e.get(field) == value]
                
                # Display filter results
                st.write(f"Found {len(filtered_entities)} entities matching your filters.")
                
                # Generate filtered CSV
                filtered_csv = entities_to_csv(filtered_entities)
                download_name = f"filtered_entities"
                if filters["provider"]:
                    download_name += f"_{filters['provider'].replace(' ', '_')}"
                if filters["facility"]:
                    download_name += f"_{filters['facility'].replace(' ', '_')}"
                ...
                if filtered_entities:
                    preview_df = pd.DataFrame([{
                        ...
                        "Provider": e.get("provider", ""),
                        "Facility": e.get("facility", "")
                    } for e in filtered_entities[:5]])
                    st.dataframe(preview_df, use_container_width=True)
                
            elif export_type == "Timeline CSV":
                st.info("Timeline CSV organizes documents by date with page and provider information.")
                timeline_csv = generate_timeline_csv(entities)
                st.download_button(
                ...
                # Preview
                st.markdown("### Preview")
                preview_df = pd.read_csv(io.StringIO(timeline_csv), nrows=5)
                st.dataframe(preview_df, use_container_width=True)
                
            elif export_type == "Lab Tests CSV":
                st.info("Lab Tests CSV extracts all laboratory tests mentioned in the document.")
                lab_tests_csv = generate_lab_tests_csv(entities)
                st.download_button(
                    label="📥 Download Lab Tests CSV",
                    data=lab_tests_csv,
                    file_name="lab_tests.csv",
                    mime="text/csv"
                )
                ...
                # Preview
                try:
                    preview_df = pd.read_csv(io.StringIO(lab_tests_csv), nrows=5)
                    st.dataframe(preview_df, use_container_width=True)
                except:
                    st.warning("No lab test data available for preview.")

            # CSV Data Schema Documentation
            with st.expander("CSV Schema Documentation"):
                st.markdown("""
                ### CSV Export Schema Documentation
                
                #### Simple CSV Format
                - **page_number**: Page where entity was found
                - **date**: Date extracted from document (possible date of service)
                - **provider**: Healthcare provider name
                - **facility**: Healthcare facility or institution name
                - **lab_tests**: Laboratory tests identified (semicolon-separated)
                - **medical_terms**: Medical terminology identified (semicolon-separated)
                
                #### Detailed CSV Format
                Contains multiple sections:
                1. **Main Entity Table**: All entities with core fields
                2. **Provider Summary**: Summary of providers with page counts
                3. **Facility Summary**: Summary of facilities with page counts
                4. **Date Summary**: Summary of dates with associated pages
                
                #### Timeline CSV Format
                - **Date**: Date extracted from document
                - **Pages**: List of page numbers associated with date
                - **Page Count**: Number of pages for this date
                - **Providers**: Healthcare providers associated with date
                - **Facilities**: Healthcare facilities associated with date
                
                #### Lab Tests CSV Format
                - **Page**: Page number where lab test was found
                - **Date**: Date associated with the lab test
                - **Provider**: Provider associated with the lab test
                - **Lab Test**: Name of laboratory test identified
                """)

        # Original Export Section (combined with new CSV exports)
        ...
        with st.expander("📁 Advanced Export Options"):
            ...
            if export_provider != "All Providers":
                ...
                # Add CSV export option for filtered provider
                provider_csv = entities_to_csv(filtered_entities)
                st.download_button(
                    label=f"📊 Export {export_provider} Records as CSV",
                    data=provider_csv,
                    file_name=f"{export_provider.replace(' ', '_')}_records.csv",
                    mime="text/csv"
                )
    else:
        st.info("👨⚕️ Upload clinical documentation to begin analysis")
        
        # Show CSV feature highlight in the landing page
        with st.expander("✨ New Feature: Advanced CSV Export"):
            st.markdown("""
            ### New CSV Export Features
            
            Our Medical Entity Extractor Pro now includes advanced CSV export capabilities:
            
            - **Simple CSV Export**: Basic entity information in tabular format
            - **Detailed CSV Export**: Comprehensive report with entity summaries and statistics
            - **Filtered CSV Export**: Create custom exports based on providers, facilities, etc.
            - **Timeline CSV Export**: Document organization by date with page mapping
            - **Lab Tests CSV Export**: Specialized extraction of laboratory test information
            
            Upload a clinical document to start using these powerful new features!
            """)
